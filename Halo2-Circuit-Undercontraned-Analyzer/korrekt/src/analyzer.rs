...
impl<'b,F: FieldExt> Analyzer<F> {
    ...
    fn control_uniqueness(
        ...
        for i in 1..=max_iterations {            
            ...
            // Instead of doing an approach with copy solver.
            // We will save state in a stack using push and pop.
            //let constrained_solver = Solver::new(&analyzer_input.z3_context);//solver;//Self::copy_solver(solver);
            ...
            solver.check_assumptions(&model_variables_assignment);
            let constrained_model = solver.get_model().unwrap();
            ...
            solver.check_assumptions(&nvc_p1);
            // 4. find a model that satisfies these rules
            if matches!(solver.check_assumptions(&nvc_p1),SatResult::Sat) {
