...
mod tests {
    ...
    use crate::sample_circuits::pse_v1 as sample_circuits;
    use halo2curves::bn256;
    use num::{BigInt, Num};
    use pse_v1_halo2_proofs::halo2curves::bn256::Fr;
    ...
    fn under_constrained_enough_random_input_test() {
        let circuit =
            sample_circuits::bit_decomposition::two_bit_decomp::TwoBitDecompCircuitUnderConstrained::<
                Fr,
            >::default();
        let k: u32 = 11;        let analyzer_input: analyzer_io_type::AnalyzerInput = analyzer_io_type::AnalyzerInput {
    ...
    fn analyze_underconstrained_multiple_interpreted_lookup_random_test() {
        ...
        let mut analyzer_setup = Analyzer::new(&circuit, k,AnalyzerType::UnderconstrainedCircuit,Some(&analyzer_input)).unwrap();        let output_status = analyzer_setup.analyzer
            .analyze_underconstrained(&analyzer_input, &mut analyzer_setup.smt_file)
            .unwrap()
            .output_status;
        assert!(output_status.eq(&AnalyzerOutputStatus::NotUnderconstrainedLocal));
