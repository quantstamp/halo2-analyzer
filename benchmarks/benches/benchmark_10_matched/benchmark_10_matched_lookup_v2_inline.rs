...
macro_rules! benchmark_with_size {
    ($c:expr, $size:expr) => {{
        let mut group = $c.benchmark_group(format!("underconstrained_10_matched_lookup_v2_inline"));
        ...
        group.bench_function(format!("size_{}", $size), |b| {
            b.iter_batched(
                || {
                    let circuit =
                        sample_circuits::pse_v1::lookup_circuits::ten_matched_lookups::MyCircuit::<
                            Fr,
                            $size,
                        >(PhantomData);
                    let k: u32 = 11;
